...
class DataGenerator(keras.utils.Sequence):
	...
	# Generate indexes of the batch
		...
		cindexes = self.channel_indexes[index*self.batch_size:(index+1)*self.batch_size]

		# Generate data
		X, Y = self.__data_generation(batch_indexes,cindexes)
	...
	def on_epoch_end(self):
		...
		self.channel_indexes = np.random.randint(self.n_channels, size=self.nsamples)
		if self.shuffle == True:
			np.random.shuffle(self.indexes)
			self.channel_indexes = np.random.randint(self.n_channels, size=self.nsamples)
		
	def __data_generation(self, batch_indexes,cindexes):
		'Generates data containing batch_size samples' # X : (n_samples, *dim, n_channels)
		# Initialization
		X = np.empty((self.batch_size, self.dim[0],self.dim[1], 2))
		mask = np.empty((self.batch_size, self.dim[0],self.dim[1], 2))
		...
		for ii in range(batch_indexes.shape[0]):
			...
			with h5py.File(self.list_IDs[file_id], 'r') as f:
				...
				if kspace.shape[2] == self.dim[1]:
					X[ii,:,:,:] = kspace[self.crop[0]+file_slice,:,:,cindexes[ii]*2:(cindexes[ii] + 1)*2]
				else:
					idx = int((kspace.shape[2] - self.dim[1])/2)
					X[ii,:,:,:] = kspace[self.crop[0]+file_slice,:,idx:-idx,cindexes[ii]*2:(cindexes[ii] + 1)*2]
		...
		X[mask] = 0
		#norm = np.abs(aux).max(axis = (1,2,3),keepdims = True) # Normalize using the maximum absolute value across real and imaginary compnents
